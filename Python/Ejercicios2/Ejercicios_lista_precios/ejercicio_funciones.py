...
def mostrar_productos(productos):
    for elem in productos:
        ...
        print(f"{elem['nombre']} {elem['precio']}")
        print("---")
...
def calcular_precio_actualizado(precioAnterior, porcentajeAumento):
    precioActualizado = precioAnterior + (precioAnterior * porcentajeAumento) / 100
...
def actualizar_precios(lista, porcentajeAumento):
    for elem in lista:
        elem['precio'] = calcular_precio_actualizado(elem['precio'], porcentajeAumento)
